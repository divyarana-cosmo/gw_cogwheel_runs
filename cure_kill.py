...
for relno in range(15,20):
    ...
    for runno in range(len(data['ra'])):
        fil = 'output/LVCPrior_uni_dl/o4sim_relno_%d_run_%d/run_*'%(relno, runno)
        flist = len(glob(fil))
        fil = 'output/LVCPrior_uni_dl/o4sim_relno_%d_run_%d/run_%d'%(relno, runno,flist-1)
        jfil = fil + '/samples.feather'        
        chk = exists(jfil)
        if not chk:
            fout.write('%d\t%d\n'%(relno,runno))
            call('rm -r output/LVCPrior_uni_dl/o4sim_relno_%d_run_%d'%(relno, runno), shell=1)
